...
def train_one_epoch(model, data_loader, device, record, epoch):
    model.train()
    calculation(model, "train", data_loader, device, record, epoch)
...
def calculation(model, mode, data_loader, device, record, epoch):
    L = len(data_loader)
    running_loss = 0.0
